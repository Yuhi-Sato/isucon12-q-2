...
func connectAdminDB() (*sqlx.DB, error) {
	...
	config.Net = "tcp"
	config.Addr = "35.79.6.218" + ":" + getEnv("ISUCON_DB_PORT", "3306")
...
// 大会ごとの課金レポートを計算する
func billingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string, scoredPlayerIDs []string) (*BillingReport, error) {
...
func competitionRankingHandler(c echo.Context) error {
	...
	if err := tenantDB.SelectContext(
		ctx,
		&psps,
		"SELECT ps.score as score, p.id as player_id, p.display_name as display_name, ps.row_num as row_num FROM player_score ps LEFT JOIN player p ON ps.player_id = p.id WHERE ps.tenant_id = ? AND ps.competition_id = ?",
		tenant.ID,
		competitionID,
	); err != nil {
	...
	playerScoreRowNumByPlayerID := map[string]int64{}
	for _, psp := range psps {
		if rowNum, ok := playerScoreRowNumByPlayerID[psp.PlayerID]; ok {
			if rowNum > psp.RowNum {
				continue
			}
		} else {
			playerScoreRowNumByPlayerID[psp.PlayerID] = psp.RowNum
		}

		filteredPlayerScorePlayers = append(filteredPlayerScorePlayers, psp)
	}

	ranks := make([]CompetitionRank, 0, len(psps))
	scoredPlayerSet := make(map[string]struct{}, len(psps))
	for _, psp := range filteredPlayerScorePlayers {
